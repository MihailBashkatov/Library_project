...
from books.models import Library, Author, BookFeature, BookGeneral, BookGenre, BookDetail, BookFinance
...
class BookDetailTestCase(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        self.genre = BookGenre.objects.create(
            genre="Kids")
    ...
    def test_book_update(self):
        ...
        data = {"book_general": self.book_general.pk, "page_amount": 300, "edition_year": "1998-01-03"}
        response = self.client.patch(url, data)
        data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class BookGeneralTestCase(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        print(f'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA GENREEEEEEEEEEEEEEEEE pk{self.genre.pk}')
    ...
    def test_book_general_create(self):
        url = reverse("books:book_general-create")
        print('URL')
        ...
        response = self.client.post(url, data)
        print(f' TEST BOOOK  General CREEATE {response.json()}')
        # self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        self.assertEqual(BookFeature.objects.all().count(), 0)

    # def test_book_update(self):
    #     url = reverse("books:book_general-update", args=(self.book_general.pk,))
    #     data = {"age_restriction": 16, "description": "New description", "genre": self.genre.pk}
    #     response = self.client.patch(url, data)
    #     data = response.json()
    #     print(f' LINEEEEEEEEEEEEEE 466 {data}')
    #     self.assertEqual(response.status_code, status.HTTP_200_OK)
    #     self.assertEqual(data.get("age_restriction"), 16)
    ...
    def test_books_general_list(self):
        ...
        response = self.client.get(url)
        data = response.json()
        print(f'LLLLLLLLLLLLLLLLLLLLLLLLLLLL {data}')
        result = {'count': 1,
         'next': None,
         'previous': None,
         'results': [
...
class BookFinanceTestCase(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        self.book_finance = BookFinance.objects.create(book=self.book,
                                                       price=100)
    ...
    def test_book_finance_create(self):
        ...
        response = self.client.post(url, data)
        data = response.json()
        print(f'ZZZZZZZZZZZZZZZZZZZZZZZZZZZZ {data}')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_books_fianance_list(self):
        url = reverse("books:book_finances-list")
        response = self.client.get(url)
        data = response.json()
        print(f'BOOOOK FINAUUUUUUUUUUUUUUUUUUUUUUUUUUUUuuUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUNACE {data}')
